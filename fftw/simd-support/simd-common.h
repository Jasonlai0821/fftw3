...
# define FFTW_NAME(name) CONCAT(fftw_, name)
/* rename for precision and for SIMD extensions */
#define XSIMD0(name, suffix) CONCAT(name, suffix)
#define XSIMD(name) XSIMD0(FFTW_NAME(name), SIMD_SUFFIX)
...
#define SIMD_STRIDE_OK(x) (!(((x) * sizeof(FFTW_REAL_TYPE)) % ALIGNMENT))
#define SIMD_STRIDE_OKA(x) (!(((x) * sizeof(FFTW_REAL_TYPE)) % ALIGNMENTA))
