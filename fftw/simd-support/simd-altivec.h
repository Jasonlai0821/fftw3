...
#  error "compiling rdft_simd-altivec.h requires -maltivec or equivalent"
#endif
...
static inline V LDA(const FFTW_REAL_TYPE *x, INT ivs, const FFTW_REAL_TYPE *aligned_like) 
{
     UNUSED(ivs);
     UNUSED(aligned_like);
     return vec_ld(0, x);
...
static inline V LD(const FFTW_REAL_TYPE *x, INT ivs, const FFTW_REAL_TYPE *aligned_like) 
{
     /* common subexpressions */
     const INT fivs = sizeof(FFTW_REAL_TYPE) * ivs;
...
/* store lower half */
static inline void STH(FFTW_REAL_TYPE *x, V v, FFTW_REAL_TYPE *aligned_like)
{
     v = vec_perm(v, v, vec_lvsr(0, aligned_like));
     vec_ste(v, 0, x);
     vec_ste(v, sizeof(FFTW_REAL_TYPE), x);
}

static inline void STL(FFTW_REAL_TYPE *x, V v, INT ovs, FFTW_REAL_TYPE *aligned_like)
{
     const INT fovs = sizeof(FFTW_REAL_TYPE) * ovs;
     v = vec_perm(v, v, vec_lvsr(fovs + 8, aligned_like));
     vec_ste(v, fovs, x);
     vec_ste(v, sizeof(FFTW_REAL_TYPE) + fovs, x);
}

static inline void STA(FFTW_REAL_TYPE *x, V v, INT ovs, FFTW_REAL_TYPE *aligned_like) 
{
     UNUSED(ovs);
     UNUSED(aligned_like);
     vec_st(v, 0, x);
...
static inline void ST(FFTW_REAL_TYPE *x, V v, INT ovs, FFTW_REAL_TYPE *aligned_like) 
{
...
static inline void STN2(FFTW_REAL_TYPE *x, V v0, V v1, INT ovs)
{
     const INT fovs = sizeof(FFTW_REAL_TYPE) * ovs;
...
static inline void STN4(FFTW_REAL_TYPE *x, V v0, V v1, V v2, V v3, INT ovs)
{
     const INT fovs = sizeof(FFTW_REAL_TYPE) * ovs;
...
static inline V BYTW1(const FFTW_REAL_TYPE *t, V sr)
{
...
static inline V BYTWJ1(const FFTW_REAL_TYPE *t, V sr)
{
     const V *twp = (const V *)t;
     V si = VBYI(sr);
     V tx = twp[0];
...
static inline V BYTW2(const FFTW_REAL_TYPE *t, V sr)
{
     const V *twp = (const V *)t;
     V si = FLIP_RI(sr);
     V tr = twp[0], ti = twp[1];
...
static inline V BYTWJ2(const FFTW_REAL_TYPE *t, V sr)
{
     const V *twp = (const V *)t;
     V si = FLIP_RI(sr);
     V tr = twp[0], ti = twp[1];
