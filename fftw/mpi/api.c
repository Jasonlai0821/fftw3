			   ...
			   int rdft2)
{
	...
	possible with as few distributed dimensions as possible. */
     ...
     /* punt for 1d prime */
     if (rnk == 1 && fftw_is_prime(sz->dims[0].n))
			      ...
			      int sign, unsigned flags)
{
	...
	from the solvers somehow? */
     ...
     if (rnk > 1 && XM(is_block1d)(sz, IB) && XM(is_block1d)(sz, OB)) {
	  ...
	  for (i = 0; i < 2; ++i)
		   ...
		   XM(num_blocks)(sz->dims[i].n, sz->dims[i].b[OB]) == 1) {
		    ...
		    N = fftw_imax(N, prod(rnk, local_n_in));
		    sz->dims[i] = odims[i];
	  ...
	  if (howmany >= n_pes && !MPI_FLAGS(flags)) { /* dft-rank1-bigvec */
	       ...
	       N = fftw_imax(N, (prod(2, n) + howmany - 1) / howmany);
	  }
	  else { /* dft-rank1 */
     ...
     N = fftw_imax(N, fftw_imax(prod(rnk, local_n_in), prod(rnk, local_n_out)));
				...
				MPI_Comm comm, unsigned flags)
{
     ...
     return 
	  fftw_mkapiplan(FFTW_FORWARD, flags,
...
X(plan) XM(plan_r2r_2d)(ptrdiff_t nx, ptrdiff_t ny, R *in, R *out,
			MPI_Comm comm,
			X(r2r_kind) kindx, fftw_r2r_kind kindy,
			unsigned flags)
{
     ptrdiff_t n[2];
...
static fftw_plan plan_guru_rdft2(int rnk, const XM(ddim) *dims0,
			       ptrdiff_t howmany,
			       R *r, C *c,
...
void XM(execute_dft)(const fftw_plan p, C *in, C *out) {
     /* internally, MPI plans are just rdft plans */
     fftw_execute_r2r(p, (R*) in, (R*) out);
}

void XM(execute_dft_r2c)(const fftw_plan p, R *in, C *out) {
     /* internally, MPI plans are just rdft plans */
     fftw_execute_r2r(p, in, (R*) out);
}

void XM(execute_dft_c2r)(const fftw_plan p, C *in, R *out) {
     /* internally, MPI plans are just rdft plans */
     fftw_execute_r2r(p, (R*) in, out);
}

void XM(execute_r2r)(const fftw_plan p, R *in, R *out) {
     /* internally, MPI plans are just rdft plans */
