...
{
     int r;
     for (r = (int) (fftw_isqrt(np)); np % r != 0; ++r)
	  ;
     return r;
}
...
{
     int r = (int) (fftw_first_divisor(np));
     return (r >= (int) (fftw_isqrt(np)) ? 0 : r);
}
...
{
     return fftw_imax(XM(block)(nx, block, pe) * ny,
...
static void awake(plan *ego_, enum wakefulness wakefulness)
{
     ...
     fftw_plan_awake(ego->cldtr, wakefulness);
     fftw_plan_awake(ego->cldtm, wakefulness);
}

static void destroy(plan *ego_)
{
     P *ego = (P *) ego_;
     fftw_plan_destroy_internal(ego->cldtm);
     fftw_plan_destroy_internal(ego->cldtr);
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     if (p->flags & TRANSPOSED_IN) { 
	  ...
	  cld1 = fftw_mkplan_f_d(plnr,
                               fftw_mkproblem_rdft_0_d(fftw_mktensor_3d
						     (m, r*vn, vn,
						      r, vn, m*vn,
						      vn, 1, 1),
                                                     I, O),
	  ...
	  INT bt = p->tblock;
	  cld1 = fftw_mkplan_f_d(plnr,
                               fftw_mkproblem_rdft_0_d(fftw_mktensor_5d
						     (b, m*r*bt*vn, vn,
						      m, r*bt*vn, bt*b*vn,
						      r, bt*vn, m*bt*b*vn,
						      bt, vn, b*vn,
	  ...
	  cld1 = fftw_mkplan_f_d(plnr,
                               fftw_mkproblem_rdft_0_d(fftw_mktensor_4d
						     (m, r*vn, vn,
						      r, vn, m*vn,
						      vn, 1, 1,
						      b, np*vn, np*vn),
     ...
     if (b)
	  cldtr = fftw_mkplan_d(plnr, XM(mkproblem_transpose)
			      (b, p->ny, p->vn,
     ...
     MPI_Comm_split(p->comm, me % r, me, &comm2);
     if (b)
	  cldtm = fftw_mkplan_d(plnr, XM(mkproblem_transpose)
			      (p->nx, b, p->vn,
     ...
     if (cldtr) fftw_ops_add2(&cldtr->ops, &pln->super.super.ops);
     if (cldtm) fftw_ops_add2(&cldtm->ops, &pln->super.super.ops);

     return &(pln->super.super);

 nada:
     fftw_plan_destroy_internal(cldtm);
     fftw_plan_destroy_internal(cldtr);
