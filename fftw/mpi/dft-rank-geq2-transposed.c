...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     fftw_extract_reim(p->sign, I = p->I, &ri, &ii);
     fftw_extract_reim(p->sign, O = p->O, &ro, &io);
     ...
     sz = fftw_mktensor(p->sz->rnk - 1); /* tensor of last rnk-1 dimensions */
     i = p->sz->rnk - 2; A(i >= 0);
     ...
     nrest = 1; for (i = 1; i < sz->rnk; ++i) nrest *= sz->dims[i].n;
     {
	  ...
	  cld1 = fftw_mkplan_d(plnr,
                             fftw_mkproblem_dft_d(sz,
                                                fftw_mktensor_2d(b, is, is,
                                                               p->vn, 2, 2),
                                                ri, ii, ro, io));
     ...
     fftw_extract_reim(p->sign, O, &ro, &io);
     {
	  ...
	  cld2 = fftw_mkplan_d(plnr,
			     fftw_mkproblem_dft_d(fftw_mktensor_1d(
						     p->sz->dims[0].n,
						     nrest * 2, nrest * 2),
     ...
     fftw_ops_add2(&cldt->ops, &pln->super.super.ops);
