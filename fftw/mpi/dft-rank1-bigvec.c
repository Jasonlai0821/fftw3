...
{
     P *ego = (P *) ego_;
     fftw_plan_awake(ego->cldt_before, wakefulness);
     fftw_plan_awake(ego->cld, wakefulness);
     fftw_plan_awake(ego->cldt_after, wakefulness);
}

static void destroy(plan *ego_)
{
     P *ego = (P *) ego_;
     fftw_plan_destroy_internal(ego->cldt_after);
     fftw_plan_destroy_internal(ego->cld);
     fftw_plan_destroy_internal(ego->cldt_before);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     cldt_before = fftw_mkplan_d(plnr,
     ...
     cld = fftw_mkplan_d(plnr,
		       fftw_mkproblem_dft_d(fftw_mktensor_1d(nx, vn*2, vn*2),
					  fftw_mktensor_2d(yb, vn*2*nx, vn*2*nx,
							 vn, 2, 2),
					  ro, io, ri, ii));
     if (XM(any_true)(!cld, p->comm)) goto nada;	  
     
     cldt_after = fftw_mkplan_d(plnr,
			      XM(mkproblem_transpose)(
				   ny, nx, vn*2,
				   I, O,
     ...
     fftw_ops_add(&cldt_before->ops, &cld->ops, &pln->super.super.ops);
     fftw_ops_add2(&cldt_after->ops, &pln->super.super.ops);

     return &(pln->super.super);

 nada:
     fftw_plan_destroy_internal(cldt_after);
     fftw_plan_destroy_internal(cld);
     fftw_plan_destroy_internal(cldt_before);
     return (plan *) 0;
}
