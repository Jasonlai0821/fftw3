...
static void awake(plan *ego_, enum wakefulness wakefulness)
{
     ...
     fftw_plan_awake(ego->cld2rest, wakefulness);
...
{
     P *ego = (P *) ego_;
     fftw_ifree0(ego->sched);
     fftw_ifree0(ego->send_block_sizes);
     MPI_Comm_free(&ego->comm);
     fftw_plan_destroy_internal(ego->cld3);
     fftw_plan_destroy_internal(ego->cld2rest);
			      ...
			      INT *rest_Ioff, INT *rest_Ooff)
{
     ...
     *rest_Ioff = *rest_Ooff = 0;
	  ...
	  *cld2 = fftw_mkplan_f_d(plnr,
				X(mkproblem_rdft_0_d)(fftw_mktensor_3d
						      (nxb, bt * b, b,
						       bt, b, nx,
						       b, 1, 1),
						      I, O),
	  ...
	  if (nxr > 0) {
	       ...
	       *rest_Ooff = nxb * b;
	       b = nxr * vn;
	       *cld2rest = fftw_mkplan_f_d(plnr,
					 fftw_mkproblem_rdft_0_d(fftw_mktensor_2d
							       (bt, b, nx,
								b, 1, 1),
							       I + *rest_Ioff,
							       O + *rest_Ooff),
...
nada:
     fftw_plan_destroy_internal(*cld3);
     fftw_plan_destroy_internal(*cld2rest);
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     n_pes = (int) fftw_imax(XM(num_blocks)(p->nx, p->block),
     ...
     if (cld1) fftw_ops_add2(&cld1->ops, &pln->super.super.ops);
     if (cld2) fftw_ops_add2(&cld2->ops, &pln->super.super.ops);
     if (cld2rest) fftw_ops_add2(&cld2rest->ops, &pln->super.super.ops);
     if (cld3) fftw_ops_add2(&cld3->ops, &pln->super.super.ops);
