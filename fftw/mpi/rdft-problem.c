...
static void hash(const problem *p_, md5 *m)
{
     ...
     int i;
     fftw_md5puts(m, "mpi-dft");
     fftw_md5int(m, p->I == p->O);
     /* don't include alignment -- may differ between processes
	X(md5int)(m, fftw_ialignment_of(p->I));
	X(md5int)(m, fftw_ialignment_of(p->O));
	... note that applicability of MPI plans does not depend
     ...
     for (i = 0; i < p->sz->rnk; ++i)
	  fftw_md5int(m, p->kind[i]);
     fftw_md5int(m, p->flags);
     MPI_Comm_size(p->comm, &i); fftw_md5int(m, i);
			    ...
			    const rdft_kind *kind, unsigned flags)
{
     ...
     ego = (problem_mpi_rdft *) fftw_mkproblem(sizeof(problem_mpi_rdft)
					     + sizeof(rdft_kind)
					     * (rnk > 0 ? rnk - 1 : 0), &padt);
...
#else
     ego = (problem_mpi_rdft *) fftw_mkproblem(sizeof(problem_mpi_rdft), &padt);
