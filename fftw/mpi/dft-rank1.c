...
{
     P *ego = (P *) ego_;
     fftw_plan_awake(ego->cldt, wakefulness);
     fftw_plan_awake(ego->cld_dft, wakefulness);
     fftw_plan_awake(ego->cld_ddft, wakefulness);

     switch (wakefulness) {
         case SLEEPY:
              fftw_triggen_destroy(ego->t); ego->t = 0;
              break;
         default:
...
{
     P *ego = (P *) ego_;
     fftw_plan_destroy_internal(ego->cldt);
     fftw_plan_destroy_internal(ego->cld_dft);
     fftw_plan_destroy_internal(ego->cld_ddft);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
	...
	as if they were ddft_first */
     ...
     O = p->O;
     cld_ddft = fftw_mkplan_d(plnr, XM(mkproblem_dft_d)(sz, rp * p->vn,
						      I, O, p->comm, p->sign,
						      RANK1_BIGVEC_ONLY));
     ...
     O = TAINT((preserve_input || (ddft_first && p->flags)) ? p->O : p->I, 
	       rp * p->vn * 2);
     fftw_extract_reim(p->sign, I, &ri, &ii);
     fftw_extract_reim(p->sign, O, &ro, &io);
     cld_dft = fftw_mkplan_d(plnr,
			X(mkproblem_dft_d)(fftw_mktensor_1d(rp, p->vn*2,p->vn*2),
					   fftw_mktensor_1d(p->vn, 2, 2),
					   ri, ii, ro, io));
     ...
     fftw_extract_reim(p->sign, p->O, &ro, &io);
     pln->roff = ro - p->O;
     pln->ioff = io - p->O;
     pln->vn = p->vn;
     pln->m = m;
     ...
     fftw_ops_add(&cld_ddft->ops, &cld_dft->ops, &pln->super.super.ops);
     if (cldt) fftw_ops_add2(&cldt->ops, &pln->super.super.ops);
     {
 ...
 nada:
     fftw_plan_destroy_internal(cldt);
     fftw_plan_destroy_internal(cld_dft);
     fftw_plan_destroy_internal(cld_ddft);
     return (plan *) 0;
}
