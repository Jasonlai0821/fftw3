...
/*-----------------------------------------------------------------------*/
/* some internal functions used by the f77 fftw */
...
/* f77 doesn't have data structures, so we have to pass iodims as
   parallel arrays */
static fftw_iodim *make_dims(int rnk, const int *n,
                                   const int *is, const int *os) {
    fftw_iodim *dims;
    int i;
    A(FINITE_RNK(rnk));
    dims = (fftw_iodim *) MALLOC(sizeof(fftw_iodim) * (unsigned) rnk, PROBLEMS);
    for (i = 0; i < rnk; ++i) {
        dims[i].n = n[i];
        dims[i].is = is[i];
        dims[i].os = os[i];
...
static fftw_r2r_kind *ints2kinds(int rnk, const int *ik) {
    if (!FINITE_RNK(rnk) || rnk == 0)
        ...
        k = (fftw_r2r_kind *) MALLOC(sizeof(fftw_r2r_kind) * (unsigned) rnk, PROBLEMS);
        /* reverse order for Fortran -> C */
        for (i = 0; i < rnk; ++i)
            k[i] = (fftw_r2r_kind) ik[rnk - 1 - i];
        return k;
    }
}
