...
/* threads.c: Portable thread spawning for loops, via the fftw_spawn_loop
...
/* Distribute a loop from 0 to loopmax-1 over nthreads threads.
   ...
   the same as the data parameter passed to fftw_spawn_loop.

   This function returns only after all the threads have completed. */
void fftw_spawn_loop(int loopmax, int nthr, spawn_function proc, void *data)
{
     int block_size;
     struct work *r;
     int i;
...
void fftw_threads_register_planner_hooks(void)
{
     os_static_mutex_lock(&install_planner_hooks_mutex); {
          if (!planner_hooks_installed) {
               ...
               fftw_set_planner_hooks(lock_planner_mutex, unlock_planner_mutex);
