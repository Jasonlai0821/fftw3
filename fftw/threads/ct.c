...
static void apply_dit(const plan *ego_, FFTW_REAL_TYPE *ri, FFTW_REAL_TYPE *ii, FFTW_REAL_TYPE *ro, FFTW_REAL_TYPE *io)
{
...
static void apply_dif(const plan *ego_, FFTW_REAL_TYPE *ri, FFTW_REAL_TYPE *ii, FFTW_REAL_TYPE *ro, FFTW_REAL_TYPE *io)
{
     const P *ego = (const P *) ego_;
     plan_dft *cld;
...
static void awake(plan *ego_, enum wakefulness wakefulness)
{
     ...
     for (i = 0; i < ego->nthr; ++i)
	  fftw_plan_awake(ego->cldws[i], wakefulness);
}
...
static void destroy(plan *ego_)
{
     ...
     for (i = 0; i < ego->nthr; ++i)
	  fftw_plan_destroy_internal(ego->cldws[i]);
     fftw_ifree(ego->cldws);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     if (plnr->nthr <= 1 || !fftw_ct_applicable(ego, p_, plnr))
          return (plan *) 0;
     ...
     for (i = 0; i < nthr; ++i) {
          fftw_ops_add2(&cldws[i]->ops, &pln->super.super.ops);
	  pln->super.super.could_prune_now_p |= cldws[i]->could_prune_now_p;
     }
     fftw_ops_add2(&cld->ops, &pln->super.super.ops);
     return &(pln->super.super);

 nada:
     if (cldws) {
	  for (i = 0; i < nthr; ++i)
	       fftw_plan_destroy_internal(cldws[i]);
	  fftw_ifree(cldws);
     }
