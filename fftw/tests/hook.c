...
extern fftw_plan the_plan; /* in bench.c */
...
static void hook(planner *plnr, plan *pln, const problem *p_, int optimalp)
{
     ...
     if (verbose > 5) {
	  printer *pr = fftw_mkprinter_file(stdout);
	  pr->print(pr, "%P:%(%p%)\n", p_, pln);
     ...
     if (paranoid) {
	  ...
	  if (bp) {
	       fftw_plan the_plan_save = the_plan;

	       the_plan = (apiplan *) MALLOC(sizeof(apiplan), PLANS);
	       the_plan->pln = pln;
	       the_plan->prb = (problem *) p_;

	       fftw_plan_awake(pln, AWAKE_SQRTN_TABLE);
...
static void paranoid_checks(void)
{
	...
	DSP I know of. */
     ...
     CK(sizeof(uintptr_t) >= sizeof(FFTW_REAL_TYPE *));

     CK(sizeof(INT) >= sizeof(FFTW_REAL_TYPE *));
}
