...
static double raerror(FFTW_REAL_TYPE *a, FFTW_REAL_TYPE *b, int n)
{
     if (n > 0) {
...
static void rarand(FFTW_REAL_TYPE *a, int n)
{
     int i;
...
/* C = A + B */
static void raadd(FFTW_REAL_TYPE *c, FFTW_REAL_TYPE *a, FFTW_REAL_TYPE *b, int n)
{
     int i;
...
/* C = A - B */
static void rasub(FFTW_REAL_TYPE *c, FFTW_REAL_TYPE *a, FFTW_REAL_TYPE *b, int n)
{
     int i;
...
static void rarolr(FFTW_REAL_TYPE *b, FFTW_REAL_TYPE *a, int n, int nb, int na, 
		   r2r_kind_t k)
{
...
static void raphase_shift(FFTW_REAL_TYPE *b, FFTW_REAL_TYPE *a, int n, int nb, int na,
...
static void rascale(FFTW_REAL_TYPE *a, FFTW_REAL_TYPE alpha, int n)
{
     int i;
...
static void cpyr(FFTW_REAL_TYPE *ra, bench_tensor *sza, FFTW_REAL_TYPE *rb, bench_tensor *szb)
{
     cpyr_closure k;
     k.k.apply = cpyr0;
     k.ra = ra; k.rb = rb;
...
static void dofft(info *nfo, FFTW_REAL_TYPE *in, FFTW_REAL_TYPE *out)
{
     cpyr(in, nfo->pckdsz, (FFTW_REAL_TYPE *) nfo->p->in, nfo->totalsz);
     after_problem_rcopy_from(nfo->p, (bench_real *)nfo->p->in);
     doit(1, nfo->p);
     after_problem_rcopy_to(nfo->p, (bench_real *)nfo->p->out);
     cpyr((FFTW_REAL_TYPE *) nfo->p->out, nfo->totalsz, out, nfo->pckdsz);
}

static double racmp(FFTW_REAL_TYPE *a, FFTW_REAL_TYPE *b, int n, const char *test, double tol)
{
...
static void impulse_response(int rnk, dim_stuff *d, FFTW_REAL_TYPE impulse_amp,
			     FFTW_REAL_TYPE *A, int N)
{
     if (rnk == 0)
	  A[0] = impulse_amp;
     else {
...
static double rlinear(int n, info *nfo, FFTW_REAL_TYPE *inA, FFTW_REAL_TYPE *inB, FFTW_REAL_TYPE *inC, FFTW_REAL_TYPE *outA,
		      FFTW_REAL_TYPE *outB, FFTW_REAL_TYPE *outC, FFTW_REAL_TYPE *tmp, int rounds, double tol)
{
     double e = 0.0;
     int j;

     for (j = 0; j < rounds; ++j) {
	  FFTW_REAL_TYPE alpha, beta;
	  alpha = mydrand();
	  beta = mydrand();
	  rarand(inA, n);
	  rarand(inB, n);
...
static double rimpulse(dim_stuff *d, FFTW_REAL_TYPE impulse_amp,
		       int n, int vecn, info *nfo, 
		       FFTW_REAL_TYPE *inA, FFTW_REAL_TYPE *inB, FFTW_REAL_TYPE *inC,
		       FFTW_REAL_TYPE *outA, FFTW_REAL_TYPE *outB, FFTW_REAL_TYPE *outC,
		       FFTW_REAL_TYPE *tmp, int rounds, double tol)
{
     double e = 0.0;
     int N = n * vecn;
     int i;
...
static double t_shift(int n, int vecn, info *nfo, 
		      FFTW_REAL_TYPE *inA, FFTW_REAL_TYPE *inB, FFTW_REAL_TYPE *outA, FFTW_REAL_TYPE *outB, FFTW_REAL_TYPE *tmp,
...
{
     FFTW_REAL_TYPE *inA, *inB, *inC, *outA, *outB, *outC, *tmp;
     info nfo;
     int n, vecn, N;
     ...
     inA = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     inB = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     inC = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     outA = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     outB = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     outC = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     tmp = (FFTW_REAL_TYPE *) bench_malloc(N * sizeof(FFTW_REAL_TYPE));
     ...
     if (!p->in_place && !p->destroy_input) {
	  ...
	  for (i = 0; i < rounds; ++i) {
	       rarand(inA, N);
	       dofft(&nfo, inA, outB);
	       cpyr((FFTW_REAL_TYPE *) nfo.p->in, totalsz_swap, inB, pckdsz_swap);
...
static void cpyr1(int n, FFTW_REAL_TYPE *in, int is, FFTW_REAL_TYPE *out, int os, FFTW_REAL_TYPE scale)
{
     int i;
     for (i = 0; i < n; ++i)
...
static void mke00(FFTW_COMPLEX *a, int n, int c)
{
     int i;
     for (i = 1; i + i < n; ++i)
	  a[n - i][c] = a[i][c];
...
static void mkre00(FFTW_COMPLEX *a, int n)
{
     mkreal(a, n);
     mke00(a, n, 0);
}

static void mkimag(FFTW_COMPLEX *a, int n)
{
     int i;
     for (i = 0; i < n; ++i)
	  c_re(a[i]) = 0.0;
...
static void mko00(FFTW_COMPLEX *a, int n, int c)
{
     int i;
     a[0][c] = 0.0;
     for (i = 1; i + i < n; ++i)
...
static void mkro00(FFTW_COMPLEX *a, int n)
{
     mkreal(a, n);
     mko00(a, n, 0);
}

static void mkio00(FFTW_COMPLEX *a, int n)
{
     mkimag(a, n);
     mko00(a, n, 1);
}

static void mkre01(FFTW_COMPLEX *a, int n) /* n should be be multiple of 4 */
{
     FFTW_REAL_TYPE a0;
     a0 = c_re(a[0]);
     mko00(a, n/2, 0);
...
static void mkro01(FFTW_COMPLEX *a, int n) /* n should be be multiple of 4 */
{
...
static void mkoddonly(FFTW_COMPLEX *a, int n)
{
     int i;
     for (i = 0; i < n; i += 2)
...
static void mkre10(FFTW_COMPLEX *a, int n)
{
     mkoddonly(a, n);
     mkre00(a, n);
}

static void mkio10(FFTW_COMPLEX *a, int n)
{
     mkoddonly(a, n);
     mkio00(a, n);
}

static void mkre11(FFTW_COMPLEX *a, int n)
{
     mkoddonly(a, n);
     mko00(a, n/2, 0);
     mkre00(a, n);
...
static void mkro11(FFTW_COMPLEX *a, int n)
{
     mkoddonly(a, n);
     mkre00(a, n/2);
     mkro00(a, n);
...
static void mkio11(FFTW_COMPLEX *a, int n)
{
     mkoddonly(a, n);
     mke00(a, n/2, 1);
     mkio00(a, n);
		  ...
		  double t[6])
{
     ...
     a = (FFTW_COMPLEX *) bench_malloc(n0 * sizeof(FFTW_COMPLEX));
     b = (FFTW_COMPLEX *) bench_malloc(n0 * sizeof(FFTW_COMPLEX));
