...
static double aerror(FFTW_COMPLEX *a, FFTW_COMPLEX *b, int n)
{
     if (n > 0) {
...
static void assign_conj(FFTW_COMPLEX *Ac, FFTW_COMPLEX *A, int rank, const bench_iodim *dim, int stride)
{
     if (rank == 0) {
          c_re(*Ac) = c_re(*A);
          c_im(*Ac) = -c_im(*A);
...
/* C = A */
void acopy(FFTW_COMPLEX *c, FFTW_COMPLEX *a, int n)
{
     int i;
...
void arol(FFTW_COMPLEX *b, FFTW_COMPLEX *a, int n, int nb, int na)
{
     ...
     for (ib = 0; ib < nb; ++ib) {
	  for (i = 0; i < n - 1; ++i)
	       for (ia = 0; ia < na; ++ia) {
		    FFTW_COMPLEX *pb = b + (ib * n + i) * na + ia;
		    FFTW_COMPLEX *pa = a + (ib * n + i + 1) * na + ia;
		    c_re(*pb) = c_re(*pa);
		    c_im(*pb) = c_im(*pa);
	       }

	  for (ia = 0; ia < na; ++ia) {
	       FFTW_COMPLEX *pb = b + (ib * n + n - 1) * na + ia;
	       FFTW_COMPLEX *pa = a + ib * n * na + ia;
	       c_re(*pb) = c_re(*pa);
	       c_im(*pb) = c_im(*pa);
	  }
     }
...
void ascale(FFTW_COMPLEX *a, FFTW_COMPLEX alpha, int n)
{
     ...
     for (i = 0; i < n; ++i) {
	  FFTW_REAL_TYPE xr = c_re(a[i]), xi = c_im(a[i]);
	       ...
	       FFTW_COMPLEX *tmp, int rounds, double tol)
{
     ...
     for (i = 0; i < vecn; ++i) {
	  FFTW_REAL_TYPE x = (sqrt(n)*(i+1)) / (double)(vecn+1);
	  for (j = 0; j < n; ++j) {
	       c_re(inA[j + i * n]) = 0;
	       c_im(inA[j + i * n]) = 0;
	       c_re(outA[j + i * n]) = x;
     ...
     for (i = 0; i < vecn; ++i) {
	  FFTW_REAL_TYPE x = (i+1) / ((double)(vecn+1) * sqrt(n));
	  for (j = 0; j < n; ++j) {
	       c_re(inA[j + i * n]) = x;
	       c_im(inA[j + i * n]) = 0;
	       c_re(outA[j + i * n]) = 0;
...
static int all_zero(FFTW_COMPLEX *a, int n)
{
     int i;
     for (i = 0; i < n; ++i)
...
static int one_accuracy_test(dofft_closure *k, aconstrain constrain,
			     int sign, int n, FFTW_COMPLEX *a, FFTW_COMPLEX *b, 
			     double t[6])
{
     double err[6];
