...
static void cpyr(FFTW_REAL_TYPE *ra, const bench_tensor *sza,
		 FFTW_REAL_TYPE *rb, const bench_tensor *szb)
{
     cpyr_closure k;
     k.k.apply = cpyr0;
     k.ra = ra; k.rb = rb;
		    ...
		    int indxa, int ondxa, int indxb, int ondxb)
{
     ...
     int scalea = k->scalea;
     int as = k->as * scalea;
     FFTW_REAL_TYPE *ra = k->ra + ondxa * scalea, *ia = k->ia + ondxa * scalea;
     FFTW_REAL_TYPE *rb = k->rb + indxb, *ib = k->ib + indxb;
     UNUSED(indxa); UNUSED(ondxb);
...
static void cpyhc2(FFTW_REAL_TYPE *ra, FFTW_REAL_TYPE *ia,
		   const bench_tensor *sza, const bench_tensor *vecsza,
		   int scalea,
		   FFTW_REAL_TYPE *rb, FFTW_REAL_TYPE *ib, const bench_tensor *szb)
{
     cpyhc2_closure k;
		     ...
		     int indxa, int ondxa, int indxb, int ondxb)
{
     ...
     int scalea = k->scalea;
     int as = k->as * scalea;
     FFTW_REAL_TYPE *ra = k->ra + indxa * scalea, *ia = k->ia + indxa * scalea;
     FFTW_REAL_TYPE *rb = k->rb + ondxb, *ib = k->ib + ondxb;
...
static void icpyhc2(FFTW_REAL_TYPE *ra, FFTW_REAL_TYPE *ia,
		    const bench_tensor *sza, const bench_tensor *vecsza,
		    int scalea,
		    FFTW_REAL_TYPE *rb, FFTW_REAL_TYPE *ib, const bench_tensor *szb)
{
...
{
     FFTW_COMPLEX *inA, *inB, *inC, *outA, *outB, *outC, *tmp;
     int n, vecn, N;
     ...
     inA = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
     inB = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
     inC = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
     outA = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
     outB = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
     outC = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
     tmp = (FFTW_COMPLEX *) bench_malloc(N * sizeof(FFTW_COMPLEX));
		    ...
		    double t[6])
{
     ...
     a = (FFTW_COMPLEX *) bench_malloc(n * sizeof(FFTW_COMPLEX));
     b = (FFTW_COMPLEX *) bench_malloc(n * sizeof(FFTW_COMPLEX));
